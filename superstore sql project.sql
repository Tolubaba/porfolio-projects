...
alter table orders$
 ...
 -- to remove the minus sign from the profit
 select abs(profit) from dbo.orders$
 alter table orders$
 add profits float
 alter table orders$
 drop column [profits]
...
select [monthname],COUNT([monthname]) as total from dbo.Orders$
group by [monthname]
order by 2 desc
...
-- to join orders table with loaction table we use the join statemenr
...
-----------------------
select * from temmpoaryordertable
...
as
begin
select * from temmpoaryordertable where [state]=@state and customerid=@customerid
end
-- how to execute a stored procedure
sporder 'florida','dc-12850'
